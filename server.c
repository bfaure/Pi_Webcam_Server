...
int open_listening_socket(int port)
{
	...
	if(setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (const void*)&optval, sizeof(int)) < 0 ){ return -1; }	// allow user to reuse same port
...
void handle_request(packet_t *recv_packet, struct sockaddr_in* client_addr, socklen_t* addrlen)
{
	...
	if (strcmp(req_type,"RRQ")==0)
	{
		...
		int f_exists = search_for_file(filename,1);
